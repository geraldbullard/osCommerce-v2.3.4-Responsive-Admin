  ...
  class d_latest_addons {
    ...
    function getOutput() {
      ...
      $output = '<div class="table-responsive brtr3 brtl3">' .
                ...
                '        <th class="text-right width-action-below-768">' . MODULE_ADMIN_DASHBOARD_LATEST_ADDONS_DATE . '</th>' .
                '      </tr>' .
                '    </thead>' .
                '    <tbody>';
      ...
      if (is_array($feed) && !empty($feed)) {
        foreach ($feed['items'] as $item) {
          $output .= '      <tr class="dataTableRow" onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);">
                              <td class="truncate-below-768"><a href="' . $item['link'] . '" target="_blank">' . $item['title'] . '</a></td>
                              <td class="text-right width-action-below-768">' . date("F j, Y", strtotime($item['pubDate'])) . '</td>
                            </tr>'; 
        }
      } else {
        $output .= '      <tr>' .
