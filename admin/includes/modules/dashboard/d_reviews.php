  ...
  class d_reviews {
    ...
    function getOutput() {
      ...
      $output = '<div class="table-responsive brtr3 brtl3">' .
                ...
                '        <th class="hide-below-768">' . MODULE_ADMIN_DASHBOARD_REVIEWS_DATE . '</th>' .
                '        <th class="hide-below-480">' . MODULE_ADMIN_DASHBOARD_REVIEWS_REVIEWER . '</th>' .
                '        <th class="hide-below-768">' . MODULE_ADMIN_DASHBOARD_REVIEWS_RATING . '</th>' .
                '        <th class="text-right width-action-below-768">' . MODULE_ADMIN_DASHBOARD_REVIEWS_REVIEW_STATUS . '</th>' .
                '      </tr>' .
                '    </thead>' .
                '    <tbody>';
      ...
      while ($reviews = tep_db_fetch_array($reviews_query)) {
        ...
        $output .= '      <tr onmouseover="rowOverEffect(this);" onmouseout="rowOutEffect(this);">' .
                   '        <td class="truncate-below-768"><a href="' . tep_href_link(FILENAME_REVIEWS, 'rID=' . (int)$reviews['reviews_id'] . '&action=edit') . '">' . $reviews['products_name'] . '</a></td>' .
                   '        <td class="hide-below-768">' . tep_date_short($reviews['date_added']) . '</td>' .
                   '        <td class="hide-below-480">' . tep_output_string_protected($reviews['customers_name']) . '</td>' .
                   '        <td class="hide-below-768">' . tep_image(HTTP_CATALOG_SERVER . DIR_WS_CATALOG_IMAGES . 'stars_' . $reviews['reviews_rating'] . '.gif') . '</td>' .
                   '        <td class="text-right width-action-below-768">' . $status_icon . '</td>' .
                   '      </tr>';
      }
