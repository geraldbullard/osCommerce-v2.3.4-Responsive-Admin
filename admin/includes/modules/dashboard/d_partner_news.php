      ...
      if (is_array($result) && !empty($result)) {
        $output = '               <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h3 class="panel-title">' . MODULE_ADMIN_DASHBOARD_PARTNER_NEWS_TITLE . '</h3>
                                    </div>
                                    <div class="panel-body">';

        foreach ($result as $p) {
          $output .= '                   <p>' .
                     '                     <a href="' . $p['url'] . '" target="_blank"><strong>' . $p['title'] . '</strong></a> (' . $p['category_title'] . ')<br />' . $p['status_update'] . 
                     '                   </p>';
        }

        $output .= '                   <p class="text-right">' .
                   '                     <a href="http://www.oscommerce.com/Services" target="_blank">' . MODULE_ADMIN_DASHBOARD_PARTNER_NEWS_MORE_TITLE . '</a>' .
                   '                   </p>' .
                   '                 </div>' . 
                   '               </div>';
      }
