...
?>                                                                            
            <div class="row mt10 mb10">
              ...
              <div class="col-lg-2 col-md-3 col-sm-4 col-xs-5">
                ...
                <?php echo tep_draw_input_field('search', '', 'class="form-control" placeholder="' . HEADING_TITLE_SEARCH . '"'); ?>
            ...
            <div class="row">
              <div class="col-lg-8">
                  ...
                  <table class="table table-hover table-striped bds1 bdsilver">
                    <thead class="bgsilver">
                      <tr>
                        ...
                        <th class="text-right hide-below-480"><?php echo TABLE_HEADING_ACCOUNT_CREATED; ?></th>
                      ...
                      <?php
                        ...
                        while ($customers = tep_db_fetch_array($customers_query)) {
                          ...
                          if (isset($cInfo) && is_object($cInfo) && ($customers['customers_id'] == $cInfo->customers_id)) {
                            ...
                            echo '          <tr onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '\'">' . "\n";
                          }
                      ?>
                        <td><i class="fa fa-user mr5"></i><?php echo $customers['customers_firstname'] . ' ' . $customers['customers_lastname']; ?></td>
                        <td class="hide-below-480 text-right"><?php echo tep_date_short($info['date_account_created']); ?></td>
                        <td class="text-right">
                        <?php 
                          if (isset($cInfo) && is_object($cInfo) && ($customers['customers_id'] == $cInfo->customers_id)) { 
                            echo '<i class="fa fa-hand-o-right blue fs16i"></i>'; 
                          } else { 
                            echo '<a href="' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID')) . 'cID=' . $customers['customers_id']) . '"><i class="fa fa-info-circle blue fs16i"></i></a>'; 
                          } 
                        ?>
                        </td>
                      </tr>
                ...
                <div class="row nmt10">
                  <div class="col-lg-6">
                    <?php echo $customers_split->display_count($customers_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_CUSTOMERS); ?>
                  </div>
                ...
                <div class="row mt10">
                  <div class="col-lg-12 text-right">
                    <button class="btn btn-primary" type="button" onclick="location.href='<?php echo tep_href_link(FILENAME_CUSTOMERS); ?>'"><i class="fa fa-close mr5"></i><?php echo IMAGE_RESET . ' ' . IMAGE_SEARCH; ?></button>
                  </div>
                </div>
                <?php
                    }
                ...
                <div class="panel panel-primary">
                  <div class="panel-heading">
                    <h3 class="panel-title">
                      <?php
                        switch ($action) {
                          case 'confirm':
                            echo TEXT_INFO_HEADING_DELETE_CUSTOMER;
                            break;
                          default:
                            echo $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname;
                            break;
                        }
                      ?>
                    </h3>
                  ...
                  <div class="panel-body">
                    <?php
                      switch ($action) {
                        case 'confirm':
                          echo tep_draw_form('customers', FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=deleteconfirm');
                          echo '  <p>' . TEXT_DELETE_INTRO . '</p>';
                          echo '  <p>' . $cInfo->customers_firstname . ' ' . $cInfo->customers_lastname . '</p>';
                          if (isset($cInfo->number_of_reviews) && ($cInfo->number_of_reviews) > 0) {
                            echo '  <p>' . tep_draw_checkbox_field('delete_reviews', 'on', true) . ' ' . sprintf(TEXT_DELETE_REVIEWS, $cInfo->number_of_reviews) . '</p>';
                          }
                          echo '  <p class="text-center">';
                          echo '  <button class="btn btn-danger mr5" type="button" onclick="location.href=\'' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '\'"><i class="fa fa-trash mr5"></i>' . IMAGE_DELETE . '</button>';
                          echo '  <button class="btn btn-default mr5" type="button" onclick="location.href=\'' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id) . '\'"><i class="fa fa-ban mr5"></i>' . IMAGE_CANCEL . '</button>';
                          echo '</form>';
                          break;
                        default:
                          if (isset($cInfo) && is_object($cInfo)) {
                            echo '<p>' . TEXT_DATE_ACCOUNT_CREATED . ' ' . tep_date_short($cInfo->date_account_created) . '</p>';
                            echo '<p>' . TEXT_DATE_ACCOUNT_LAST_MODIFIED . ' ' . tep_date_short($cInfo->date_account_last_modified) . '</p>';
                            echo '<p>' . TEXT_INFO_DATE_LAST_LOGON . ' '  . tep_date_short($cInfo->date_last_logon) . '</p>';
                            echo '<p>' . TEXT_INFO_NUMBER_OF_LOGONS . ' ' . $cInfo->number_of_logons . '</p>';
                            echo '<p>' . TEXT_INFO_COUNTRY . ' ' . $cInfo->countries_name . '</p>';
                            echo '<p>' . TEXT_INFO_NUMBER_OF_REVIEWS . ' ' . $cInfo->number_of_reviews . '</p>';
                            echo '<p class="text-center">';
                            echo '  <button class="btn btn-primary mr5" type="button" onclick="location.href=\'' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=edit') . '\'"><i class="fa fa-edit mr5"></i>' . IMAGE_EDIT . '</button>';
                            echo '  <button class="btn btn-danger mr5" type="button" onclick="location.href=\'' . tep_href_link(FILENAME_CUSTOMERS, tep_get_all_get_params(array('cID', 'action')) . 'cID=' . $cInfo->customers_id . '&action=confirm') . '\'"><i class="fa fa-trash mr5"></i>' . IMAGE_DELETE . '</button>';
                            echo '</p>';
                            echo '<p class="text-center">';
                            echo '  <button class="btn btn-success mr5" type="button" onclick="location.href=\'' . tep_href_link(FILENAME_ORDERS, 'cID=' . $cInfo->customers_id) . '\'"><i class="fa fa-tag mr5"></i>' . IMAGE_ORDERS . '</button>';
                            echo '  <button class="btn btn-default" type="button" onclick="location.href=\'' . tep_href_link(FILENAME_MAIL, 'customer=' . $cInfo->customers_email_address) . '\'"><i class="fa fa-envelope mr5"></i>' . IMAGE_EMAIL . '</button>';
                            echo '</p>';
                          }
                          break;
                      }
